	   ...
	   double *sdraw, double *trdraw, double *tedraw, int *vcdraw)
{
   ...
   if(binary) {
	 ...
	 Y[i+1] = 2*(iYDat[i]) - 1.0 - binary_offset; //guess at good starting for latents
      ...
      for(int i=0;i<NumObs;i++) Y[i+1] = iYDat[i]; //: training Y values.
   }
   ...
   */
   ...
   //double** mtestFits = Lib::almat(NTree,nrowTest);
    ...
    {
        for(int j=1;j<=nrowTest;j++) mtestFits[j][i] =0.0;
    }
   ...
   for (int k=1;k<=ndPost;k++) {
      ...
      if(*verbose && (k%printevery== 0)) Rprintf("iteration: %d (of %d)\n",k,ndPost);
       ...
       for(nvs i=1;i<theTrees.size();i++) {
         ...
         alpha =  Metrop(&theTrees[i],&Done,&step);
           // metropolis step for a tree i? changes tree by pointer. i think can ignore alpha.

           // need a repeat / until loop around call to Metrop().  Need two new arguments: a set of
           // probabilities of the form p(m_ij | a', c_i), i=1..N, j=1..?5?, and tau
           // need to find a way for a give row m_ij, a', c_i, evaluate predicted Y given current trees.
           // evaluate sum over ij, {predicted Y under A - predicted Y under A'} p(m_ij | a', c_i) 1/N <= tau.
           // if new tree obeys this, break loop, otherwise continue loop and do a new Metrop() call.
	 ...
	 if(k%keepevery==0)
           ...
           //mfits is ?predicted y value for row j=1..NumObs?

           //the following loop: add current tree fit subtract previous fit for tree i.
...
extern "C" {
    void mbart2(int *iNumObs, int *iNumX, int *inrowTest,
                double *iXDat, double *iYDat,
                double *iXTest,
                double *tau_U,
                double *tau_L,
                ...
                double *ipower, double *ibase,
                double *ibinary_offset,
                int *iNTree, int *indPost, int *inSkip,
                int *iprintevery, int *ikeepevery, int *ikeeptrainfits,
                int *inumcut, int *iusequants, int *iprintcutoffs,
                int *verbose,
                double *sdraw, double *trdraw, double *tedraw, int *vcdraw)
    {
        ...
        for(int i=1;i<=nrowTest;i++)
        {
            for(int j=1;j<=NTree;j++) mtestFits[i][j] =0.0;
        }
        ...
        double **old_mfits;
        old_mfits = new dp[3];
        old_mfits[1] = new double[NumObs+1];
        old_mfits[2] = new double[nrowTest+1];
        
        double* total_check = new double[nrowTest+1];
        for(int i=1;i<=nrowTest;i++) total_check[i] = 0;
        ...
        // we should skip the first nSkip --> no we shouldn't!
                
                counter = 0;
                do {
                    counter += 1;
                    // set: old_mfits = mfits;
                    for(int l = 1; l < NumObs; l++){
                        old_mfits[1][l] = mfits[1][l];
                        old_mfits[2][l] = mfits[2][l];
                        old_mfits[2][l + NumObs] = mfits[2][l + NumObs];
                    }
                    ...
                    for(int l = 1; l < NumObs; l++){
                        ...
                        for(int t = 1; t <= NTree; t++){
                            testfit_t += mtestFits[l][t];
                            testfit_c += mtestFits[l+NumObs][t];
                            //Rprintf("INNER LOOP: %f, %f\n", mtestFits[l][t], mtestFits[l+NumObs][t]);
                        }
                        // do we want +0.5 here?
                        direct_t += phi(testfit_t) * mass[l];
                        direct_c += phi(testfit_c) * mass[l];
                        //Rprintf("IN LOOP: %f, %f, %f, %f, %f, %f, %f\n \n", direct_t, direct_c, weights[l], phi(testfit_t), phi(testfit_c), testfit_t, testfit_c);
                    }
                    
                    //Rprintf("%d, %d, direct c and t: %f %f\n \n", counter, NumObs, direct_c, direct_t);
                    
                    direct_t /= NumObs;
                    direct_c /= NumObs;
                    
                    odds_ratio = (direct_t/(1-direct_t))/(direct_c/(1-direct_c));
                    //Rprintf("%d, odds ratio is: %f, %f, %f \n", counter, odds_ratio, direct_t, direct_c);
                    
                    if ( odds_ratio > t_U || odds_ratio < t_L) {
                        // set: mfits = old_mfits;
                        for(int l = 1; l < NumObs; l++){
                            mfits[1][l] = old_mfits[1][l];
                            mfits[2][l] = old_mfits[2][l];
                            mfits[2][l + NumObs] = old_mfits[2][l + NumObs];
                        }
                    }
                    
                } while ( odds_ratio > t_U || odds_ratio < t_L );
...
//moved from inside loop
            ...
            for(int l = 1; l < 4; l++){
                ...
                Rprintf("%i, mtestfit[%i] = %f, mtestfit[%d], %f \n", l, l, testfit_t, l+NumObs, testfit_c);
            }
            */
            // -----------------------
